...
export class StatisticsService {
    ...
    async getStatistics(username, options) {
        ...
        switch (user.role) {
            ...
            case 'assistant':
            default:
                throw new WrongRoleError('У администраторов и ассистентов нет статистики');
        }
    }
    async getTeacherStatistics(teacherId, from, to, type) {
        ...
        const worksSolvedPerDayPlot = this.plotService.generatePlot('Сдано работ в день', worksSolvedPerDay, 'secondary', (e) => e.date, (e) => parseInt(e.count));
        return {
            sections: [
                {
                    name: 'Пользователи',
                    ...
                    entries: [
                        ...
                        },
                        {
                            ...
                            description: 'Количество пользователей, сделавших запрос за последние 15 минут',
        ...
        return {
            sections: [
                ...
                },
                {
                    ...
                    description: 'Статистика по работам по месяцам. Показывает средний балл по работам (в %) за каждый месяц',
                    plots: [monthPlot],
                    entries: [],
                },
            ],
    ...
    getDateRange(prop, from, to, precision = 'day') {
        switch (precision) {
            case 'day':
